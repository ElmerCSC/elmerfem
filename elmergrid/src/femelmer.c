   ...
   */
{
  ...
  if (info)
    printf("Allocating for %d knots and %d elements.\n",
           noknots, noelements);
  AllocateKnots(data);
  ...
  if (info)
    printf("Reading %d element topologies.\n", noelements);
  ...
  if (info)
    printf("Reading %d degrees of freedom for %d knots.\n",
           novctrs, noknots);
  if (timesteps <= 1)
  {
    for (i = 1; i <= noknots; i++)
    ...
    for (k = 0; k < timesteps; k++)
    {
      ...
      if (0)
        printf("Loading steps i=%d  j=%d  k=%d  r=%.3g\n", i, j, k, r);
...
#define LONGLINE 2048
  ...
  {
    if (info)
      printf("Saving results in the interval from %d to %d with step %d\n", minstep, maxstep, dstep);
  }

  for (i = 0;; i++)
  {
    sprintf(filename, "%s.ep.%d", prefix, i);
    if ((intest = fopen(filename, "r")) == NULL)
      break;
    if (i <= MAXPARTITIONS)
      in[i] = intest;
  }
  ...
  {
    if (parts > 0)
      nofiles = MIN(parts, nofiles);
    if (info)
      printf("Loading Elmer results from %d partitions.\n", nofiles);
  }
  ...
  if (info)
    printf("There are altogether %d nodes and %d elements.\n", totknots, sumelements);

  AddExtension(outfile, filename, "ep");
  if (info)
    printf("Saving ElmerPost data to %s.\n", filename);
  ...
  if (info)
    printf("Reading and writing %d coordinates.\n", totknots);
  ...
  if (info)
    printf("Reading and writing %d element topologies.\n", totelements);
  sumknots = 0;
  ...
  if (info)
    printf("Reading and writing %d degrees of freedom.\n", novctrs);
  sprintf(outstyle, "%%.%dg ", decimals);

  activestep = FALSE;
  if (maxstep)
    timesteps = MIN(timesteps, maxstep);
  ...
  if (info)
    printf("Successfully fused partitioned Elmer results\n");
    ...
    if (elemind > 0)
    {
      ...
      for (normal = 1; normal >= -1; normal -= 2)
      {
        ...
        for (side = 0; side < elemsides; side++)
        {
          ...
          if (sideelemtype2 < 300 && sideelemtype > 300)
            break;
          if (sideelemtype2 < 200 && sideelemtype > 200)
            break;
          ...
          {
            if (debug)
              printf("sidenode: %d %d %d\n", j, sideind[j], sideind2[j]);

            hit = TRUE;
            for (i = 0; i < sidenodes; i++)
              if (sideind[(i + j) % sidenodes] != sideind2[i])
                hit = FALSE;
   ...
   */
{
  ...
  if (activeperm)
  {
    ...
    if (info)
      printf("LoadElmerInput: Node index range is: [%d %d]\n", mini, maxi);
  ...
  if ((in = fopen(filename, "r")) == NULL)
  {
    ...
    printf("Loading %d bulk elements from %s\n", noelements, filename);
  ...
  if (activeelemperm)
  {
    ...
    if (info)
      printf("LoadElmerInput: Element index range is: [%d %d]\n", minelem, maxelem);
  ...
  {
    if (info)
      printf("Loading %d boundary elements from %s\n", nosides, filename);
  }
  ...
  if (in = fopen(filename, "r"))
  {
    ...
    if (info)
      printf("Loading names for mesh parts from file %s\n", filename);
    ...
    for (;;)
    {
      ...
      {
        if (info)
          printf("Reading names for mesh boundaries\n");
        isbody = FALSE;
        continue;
      }
      ...
      {
        if (info)
          printf("Reading names for mesh bodies\n");
        isbody = TRUE;
        continue;
      }
  ...
  if (info)
    printf("Elmer mesh loaded successfully\n");
   ...
   */
{
  ...
  if (info)
    printf("Saving %d node coordinates.\n", noknots);
...
#define MAXELEMENTTYPE 827
{
  ...
  if (info)
    printf("Saving mesh in ElmerSolver format to directory %s.\n",
           directoryname);
  ...
  if (info)
    printf("Saving %d coordinates to %s.\n", noknots, filename);
  if (out == NULL)
  {
    printf("opening of file was not successful\n");
    return (2);
  ...
  out = fopen(filename, "w");
  if (info)
    printf("Saving %d element topologies to %s.\n", noelements, filename);
  if (out == NULL)
  {
    printf("opening of file was not successful\n");
    return (3);
  ...
  out = fopen(filename, "w");
  if (info)
    printf("Saving boundary elements to %s.\n", filename);
  if (out == NULL)
  {
    printf("opening of file was not successful\n");
    return (4);
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      if (bound[j].types[i] < MAXBCS)
        usedbc[bound[j].types[i]] += 1;
  ...
  if (data->boundarynamesexist || data->bodynamesexist)
  {
    ...
    out = fopen(filename, "w");
    if (info)
      printf("Saving entities info to %s.\n", filename);
    if (out == NULL)
    {
      printf("opening of file was not successful\n");
      return (5);
  ...
  if (data->nodepermexist)
  {
    ...
    if (info)
      printf("Saving initial node permutation to %s.\n", filename);
    if (out == NULL)
    {
      printf("opening of file was not successful\n");
      return (3);
                 ...
                 char *prefix, int info)
{
  ...
  if (info)
    printf("Saving size info into file %s.\n", filename);
   ...
   */
{
  ...
  if (data->nodeconnectexist)
    smallerror("Connectivity data is not saved in the FEM/BEM version");
  if (data->nopartitions > 1)
    smallerror("Partitioning data is not saved in the FEM/BEM version");
  ...
  if (info)
    printf("Saving %d (of %d) body elements to mesh.boundary\n", nobulkelements, noelements);
  ...
  fclose(out);
  if (info)
    printf("Saving %d boundary elements to mesh.boundary\n", sumsides);
  ...
  out = fopen(filename, "w");
  if (info)
    printf("Saving header info with %d elementtypes to %s.\n", tottypes, filename);
  if (out == NULL)
  {
    printf("opening of file was not successful\n");
    return (4);
  ...
  if (info)
    printf("Body and boundary numbers were permutated\n");
...
static int CreatePartitionTable(struct FemType *data, int info)
{
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    for (j = 0; j < nonodes; j++)
    {
      ...
      debug = FALSE;
      if (debug)
        printf("ind=%d i=%d j=%d part=%d\n", ind, i, j, part);

      hit = 0;
      for (k = 1; k <= maxneededtimes; k++)
      {
        if (data->partitiontable[k][ind] == 0)
          hit = k;
        if (data->partitiontable[k][ind] == part)
          hit = -k;
        if (hit)
          break;
      }
...
omstart:
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    maxpart = maxpart2 = 1;
    for (j = 1; j <= nopartitions; j++)
      if (nodesinpart[j] > nodesinpart[maxpart])
        maxpart = j;
    if (maxpart == 1)
      maxpart2 = 2;
    for (j = 1; j <= nopartitions; j++)
    {
      if (j == maxpart)
        continue;
      if (nodesinpart[j] > nodesinpart[maxpart2])
        maxpart2 = j;
    }
   ...
   This is optimal for Elmer since the elemental partitioning is primary. */
{
  ...
  if (info)
    printf("Number of nodal partitions: %d\n", nopartitions);
...
omstart:
  ...
  for (i = 1; i <= noknots; i++)
  {
    ...
    else if (nodesinpart[maxpart] > nodesinpart[maxpart2])
    {
      if (set)
        nodepart[i] = maxpart;
      else
   ...
   This may be the optimal method of partitioning for simple geometries. */
{
  ...
  if (data->elemconnectexist || data->nodeconnectexist)
  {
    ...
    {
      if (elemconnect[i])
        noelements0 = noelements0 + 1;
      noparts0 = MAX(noparts0, elemconnect[i]);
    }
    if (info)
      printf("There are %d initial partitions in the connected mesh\n", noparts0);
    if (info)
      printf("There are %d initial elements in the connected mesh\n", noelements0);
  }
  ...
  if (periodic)
  {
    if (dimper[0] && partitions1 > 1)
      vpartitions1 *= 2;
    if (dimper[1] && partitions2 > 1)
      vpartitions2 *= 2;
    if (dimper[2] && partitions3 > 1)
      vpartitions3 *= 2;
  }
  ...
  {
    if (info)
      printf("Only one regular partitions requested, skipping 2nd part of hybrid partitioning\n");
  ...
  if (vpartitions1 > 1)
  {
    ...
    if (info)
      printf("Ordering 1st direction with (%.3g*x + %.3g*y + %.3g*z)\n", cx, cy, cz);
  ...
  {
    if (info)
      printf("Ordering in the 2nd direction.\n");
  ...
  {
    if (info)
      printf("Ordering in the 3rd direction.\n");
...
skippart:
  ...
  if (info)
    printf("Successfully made a partitioning with %d to %d elements.\n", minpart, maxpart);
   ...
   An example would be L-type geometry where 3/4 of potential partitions would be active. */
{
  ...
  j = 0;
  if (info)
    printf("Renumbering the partitions (showing only 64):\n");
  for (i = 1; i <= partitions; i++)
  {
    k = nopart[i];
    if (k)
  ...
  if (info)
    printf("There are %d active partitions out of %d possible ones\n", j, partitions);
                                      ...
                                      int info)
{
  ...
  for (i = 1; i <= MAXCATEGORY; i++)
  {
    ...
    if (dim == 3)
      cumz[i] = ceil(1.0 * partitions3 * cumz[i] / j);
  }
  ...
  {
    if (info)
      printf("All possible %d out of %d partitions are active\n", j, partitions);
    data->nopartitions = partitions;
  }
                                       ...
                                       struct ElmergridType *eg, int info)
{
  ...
  {
    if (bound[bc].created == FALSE)
      continue;
    if (bound[bc].nosides == 0)
      continue;
  ...
  if (info)
    printf("Number of constrained bulk elements = %d\n", totpartelems);
  if (info)
    printf("Successfully made connected elements to a partiotion\n");
...
omstart:
  ...
  for (bc = 0; bc < MAXBOUNDARIES; bc++)
  {
    ...
    for (i = 1; i <= bound[bc].nosides; i++)
    {
      ...
      for (k = 1; k <= eg->connect; k++)
      {
        ...
        if (bctype > 0)
        {
          if (bound[bc].types[i] == bctype)
            hit = TRUE;
        }
        else if (bctype == -1)
        {
          if (bound[bc].parent[i])
            hit = TRUE;
        }
        else if (bctype == -2)
        {
          if (bound[bc].parent[i] && bound[bc].parent2[i])
            hit = TRUE;
        }
        else if (bctype == -3)
        {
  ...
  if (!allocated)
  {
    ...
    allocated = TRUE;
    if (info)
      printf("Allocated and now setting the entries\n");
    goto omstart;
  }
  ...
  if (info)
    printf("Successfully made a partitioning 1D with %d elements\n", totpartelems);
                                    ...
                                    int nparts, int metisopt, int info)
{
  ...
  if (info)
    printf("Number of connected nodes is %d\n", noconnect);
  ...
  if (info)
    printf("Permuted the connected nodes\n");
          ...
          else if (step == 2)
          {
	       ...
	       this is the dual graph. */
            for (k = 1; k <= invmaxcon; k++)
            {
              ...
              {
                if (bcdualgraph[l][bcelem] == bcelem2)
                  hit = TRUE;
                if (bcdualgraph[l][bcelem] == 0)
                  break;
              }
              if (!hit)
              {
                if (l > dualmaxcon)
                {
                  ...
                  if (0)
                    printf("allocating dual topo %d %d\n", dualmaxcon, maxbcelem);
  ...
  {
    if (info)
      printf("Starting graph partitioning METIS_PartGraphRecursive.\n");
  ...
  {
    if (info)
      printf("Starting graph partitioning METIS_PartGraphKway.\n");
    ncon = 0;
    wgtflag = 0;
  ...
  if (0)
    printf("Finished Metis routine for boundary partitioning\n");
  ...
  data->elemconnectexist = totpartelems;
  if (info)
    printf("Successfully created boundary partitioning for %d bulk elements\n", totpartelems);
                               ...
                               int elemlayers, int info)
{
  ...
  if (info)
    printf("Initial partitioning with %d elements:\n", totpartelems);
  ...
  for (m = 1; m <= elemlayers; m++)
  {
    ...
    if (info)
      printf("Setting partition for %d elements in layer %d\n", n, m);
    if (n == 0)
    {
  ...
  if (info)
    printf("Extended partitioning of %d elements:\n", totpartelems);
  minpartelems = maxpartelems = partelems[1];
  for (j = 1; j <= nparts; j++)
  {
    minpartelems = MIN(minpartelems, partelems[j]);
  ...
  if (info)
    printf("Extended partitioning has %d to %d elements in partition\n", minpartelems, maxpartelems);

  data->elemconnectexist = totpartelems;
  free_Ivector(partelems, 1, nparts);

  if (info)
    printf("Successfully extended the partitioning by %d layers\n", elemlayers);
   ...
   elemental ordering is primary in Elmer. */
{
  ...
  if (info)
    printf("Making a simple partitioning for %d nodes in %d-dimensions.\n",
           noknots, data->dim);
  ...
  if (info)
    printf("Successfully made a partitioning with %d to %d nodes.\n", minpart, maxpart);
   ...
   used. If the elements are higher order nodal elements then use only the linear basis. */
{
  ...
  options[METIS_OPTION_DBGLVL] = 3;
  if (eg->metisseed)
    options[METIS_OPTION_SEED] = eg->metisseed;

  if (info)
    printf("Making a Metis partitioning for %d elements in %d-dimensions.\n",
  ...
  {
    if (info)
      printf("There seems to be periodic boundaries\n");
    indxper = data->periodic;
  }
  ...
  {
    if (info)
      printf("Starting graph partitioning METIS_PartMeshDual.\n");
    METIS_PartMeshDual(&ne, &nn, eptr, metistopo, NULL, NULL, &ncommon,
                       &nparts, NULL, options, &edgecut, epart, npart);
    if (info)
      printf("Finished graph partitioning METIS_PartMeshDual.\n");
  }
  else
  {
    if (info)
      printf("Starting graph partitioning METIS_PartMeshNodal.\n");
    METIS_PartMeshNodal(&ne, &nn, eptr, metistopo, NULL, NULL,
                        &nparts, NULL, options, &edgecut, epart, npart);
    if (info)
      printf("Finished graph partitioning METIS_PartMeshNodal.\n");
  }
  ...
  {
    if (info)
      printf("Set the partition given by Metis for each node\n");
    for (i = 1; i <= noknots; i++)
    {
      if (periodic)
        ...
        j = i;
      if (!j)
        printf("Cannot set partitioning for node %d\n", i);
  ...
  if (info)
    printf("Successfully made a Metis partition using the element mesh.\n");
   ...
   the ownership of the nodes will follow. The latter is optimal for Elmer. */
{
  ...
  if (info)
    printf("Making a Metis partitioning for %d nodes in %d-dimensions.\n",
  ...
  if (dual)
  {
    ...
    {
      if (info)
        printf("Just one partition left, skipping 2nd part of hybrid partitioning!\n");
      goto skippart;
    }
  ...
  if (info)
    printf("There are %d connections altogether in the graph.\n", totcon);
  ...
  if (!dual)
  {
    ...
    {
      if (info)
        printf("Creating weight for %d connections.\n", totcon);
      wgtflag = 1;
    ...
    {
      if (info)
        printf("Setting periodic connections to dominate %d\n", totcon);
      for (i = 0; i < noknots; i++)
      {
  ...
  if (info)
    printf("Finished Metis graph partitioning call.\n");

  free_Ivector(adjncy, 0, totcon - 1);
  free_Ivector(xadj, 0, nn);
  if (wgtflag == 1)
    free_Ivector(adjwgt, 0, totcon - 1);
...
static void CheckPartitioning(struct FemType *data, int info)
{
  ...
  if (info)
    printf("Checking for partitioning\n");
  ...
  if (0)
    printf("Checking that each node in elements belongs to nodes\n");
  for (i = 1; i <= data->noelements; i++)
  {
    part = elempart[i];
    elemtype = data->elementtypes[i];
  ...
  if (0)
    printf("Checking that each node in partition is shown in partition list\n");
  for (i = 1; i <= data->noknots; i++)
  {
    part = nodepart[i];
...
int OptimizePartitioningAtBoundary(struct FemType *data, struct BoundaryType *bound, int info)
{
  ...
  if (info)
    printf("Optimizing the partitioning at boundaries.\n");
	   ...
	   oscillating solutions. */
        ...
        if (k < 5)
        {
          ...
          for (l = 0; l < nodesd2; l++)
          {
            ...
            if (data->nodepart[ind] == part1)
              hit1++;
            if (data->nodepart[ind] == part2)
              hit2++;
          }
  ...
  if (info)
    printf("Ownership of %d parents was changed at BCs\n", j);
  ...
  if (0)
    printf("The partitioning at discontinuous gaps was optimized.\n");
  return (0);
}
...
static void Levelize(int n, int level, int *maxlevel, int *levels, int *rows, int *cols, int *done)
{
  ...
  *maxlevel = MAX(*maxlevel, level);
  ...
  for (j = rows[n]; j < rows[n + 1]; j++)
  {
    k = cols[j];
    if (!done[k])
      Levelize(k, level + 1, maxlevel, levels, rows, cols, done);
  }
}
   ...
   difference for normal hardware configurations. */
{
  ...
  {
    if (info)
      printf("Successful ordering: moving partitions to new positions\n");
    nodepart = data->nodepart;
    elempart = data->elempart;
    for (i = 1; i <= noelements; i++)
   ...
   to which extent these checks are needed in the current Elmer version. */
{
  ...
  partitions = data->nopartitions;
  if (info)
    printf("Optimizing for %d partitions\n", partitions);
...
optimizeownership:
    ...
    for (n = 1; n <= maxrounds; n++)
    {
      ...
      for (i = 1; i <= noknots; i++)
      {
	   ...
	   to avoid the nasty multiply coupled nodes. */
        ...
        if (abs(neededvector[j] - neededvector[k]) < dtarget)
          continue;
      ...
      if (info && nochanges)
        printf("Changed the ownership of %d nodes\n", nochanges);
  ...
  if (info)
    printf("Checking for problematic sharings\n");
  m = 0;

  if (partitions > 2)
  {
    if (info)
      printf("Optimizing sharing for %d partitions\n", partitions);
      ...
      for (i = 1; i <= noelements; i++)
      {
	   ...
	   i.e. two partitions are joined at an element but not at the same node. */
        ...
        if (hit)
        {
          ...
          {
            if (hit + m > 2)
              printf("Partitions %d and %d in element %d (%d owners) oddly related %d times\n",
                     i1, i2, i, owners, hit);
          }
  ...
  {
    if (info)
      printf("Partitioning was not altered\n");
  }
   ...
   */
{
  ...
  if (info)
  {
    ...
    if (subparts)
      printf("There are %d subpartitions\n", subparts);
  }
  ...
  if (reorder)
  {
    ...
    for (i = 1; i <= noknots; i++)
      invorder[order[i]] = i;
    if (info)
      printf("Created continuous parallel ordering\n");
  }
  ...
  if (info)
    printf("Number of boundary nodes at the boundary: %d\n", nobcnodes);
  ...
  if (info)
    printf("Number of additional interface nodes: %d\n", nobcnodes);
...
#endif
  if (info && !fail)
    printf("Created mesh directory: %s\n", directoryname);
  fail = chdir(directoryname);
#ifdef MINGW32
  ...
  {
    if (info)
      printf("Reusing existing subdirectory: %s\n", subdirectoryname);
    if (nooverwrite)
    {
      printf("Mesh seems to already exist, writing is cancelled!\n");
      return (0);
  ...
  if (info)
    printf("Saving mesh in parallel ElmerSolver format to directory %s/%s.\n",
           directoryname, subdirectoryname);

  filesetsize = MAXPARTITIONS;
  if (partitions > filesetsize)
    if (info)
      printf("Saving %d partitions in maximum sets of %d\n", partitions, filesetsize);
...
next_elements_set:
	       ...
	       first test */
            for (l = 1; l <= neededtimes[sideind[0]]; l++)
            {
              ...
              if (elementhalo[part2] == i)
                continue;
              if (part2 < partstart || part2 > partfin)
                continue;
      ...
      if (halomode == 2 && hasbcnodes)
      {
        for (j = 0; j < nodesd2; j++)
        {
          ...
          for (k = 1; k <= neededtimes[ind]; k++)
          {
            ...
            if (0)
              printf("Adding bc halo for partition %d and element %d\n", part2, i);
  ...
  {
    if (info)
      printf("There are %d bulk elements in the halo.\n", halobulkelems);
  }
  ...
  if (anyparthalo)
  {
    ...
    if (info)
      printf("There are %d additional shared nodes resulting from the halo.\n", halonodes);
  }
  else
  {
  ...
  if (indirect)
  {
    ...
    for (j = 0; j < MAXBOUNDARIES; j++)
      for (i = 1; i <= bound[j].nosides; i++)
        if (bound[j].types[i] > indirecttype)
          indirecttype = bound[j].types[i];
    indirecttype++;
    if (info)
      printf("Indirect connections given index %d and elementtype 102.\n", indirecttype);
  }
  ...
  if (info)
    printf("Saving mesh for %d partitions\n", partitions);
...
next_shared_set:
  ...
  for (l = 1; l <= noknots; l++)
  {
	...
	fprintf(outfiles[nofile],"\n");
      ...
      for (m = 1; m <= neededtimes2[i]; m++)
        if (data->partitiontable[m][i] != ownerpart[i])
          fprintf(outfiles[nofile], " %d", data->partitiontable[m][i]);
      fprintf(outfiles[nofile], "\n");
      /* } */
    }
  }
  ...
  for (part = 1; part <= partitions; part++)
  {
	   ...
	   Then it will be always saved - no matter of the halos. */
        ...
        if (trueparent || trueparent2)
        {
          ...
          if (parent && !trueparent)
          {
            ...
            if (!parthalo[1] && !parthalo[2] && !parthalo[4])
              parent = 0;
          }
        ...
        if (!saveelem && parthalo[3] && part <= subparts)
        {
          ...
          for (leftright = -1; leftright <= 1; leftright += 2)
          {
            ...
            if (part2 < 1 || part2 > subparts)
              continue;
            if (parent)
              trueparent = (elempart[parent] == part2);
            if (parent2)
              trueparent2 = (elempart[parent2] == part2);
    ...
    findindirect:
      ...
      if (0)
        printf("Number of non-element connections is %d\n", nodesides);
      ...
      if (0)
        printf("Maximum number of node-to-node connections %d\n", maxnodeconnections);
      ...
      if (0)
        printf("Number of nodes with non-element connections %d\n", connectednodes);
      ...
      for (i = 1; i <= connectednodes; i++)
      {
        for (j = 1; j <= maxnodeconnections; j++)
          for (k = j + 1; k <= maxnodeconnections; k++)
          {
            ...
            if (!nodepairs[ind][1] || !nodepairs[ind][2])
              continue;
  ...
  if (anyparthalo)
    free_Ivector(neededtimes2, 1, noknots);

  cdstat = chdir("..");
  cdstat = chdir("..");

  if (reorder)
    free_Ivector(order, 1, noknots);
...
/* Calls the fill reduction ordering algorithm of Metis library. */
{
  ...
  if (info)
    printf("Reordering %d knots and %d elements using Metis reordering routine.\n",
           noknots, noelements);
  perm = NULL;
  i = CalculateIndexwidth(data, FALSE, perm);
  if (info)
    printf("Indexwidth of the original node order is %d.\n", i);
  ...
  if (info)
    printf("Starting Metis reordering routine.\n");

  METIS_NodeND(&nn, xadj, adjncy, &numflag, &options[0], perm, iperm);

  if (info)
    printf("Finished Metis reordering routine.\n");

  if (info)
    printf("Moving knots to new positions\n");
  ...
  if (info)
    printf("Chanching the element topology\n");
  ...
  if (0)
    printf("Deallocating vectors needed for reordering.\n");
