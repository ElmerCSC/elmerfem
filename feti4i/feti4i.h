 ...
 * matrix_type: matrix format/type (see comments in caller)
 * indexBase: index base (0/1)
 */
void FETI4ICreateStiffnessMatrix(
    	FETI4IMatrix   *matrix,
    	FETI4IInt		matrix_type,
    	FETI4IInt		indexBase
);

/* Add element described by per-element data
 * elementType: type/dimension of element
 * n: number of local nodes
 * nInd: per-node index offsets (array)
 * nEntries: number of entries in indices/values
 * indices: integer indices array
 * values: corresponding real values
 */
void FETI4IAddElement(   //TODO CRS or CCS ?
    	FETI4IMatrix 	matrix,
    	FETI4IInt 		elementType,
    	FETI4IInt 		n,
    	FETI4IInt* 		nInd,
    	FETI4IInt 		nEntries,
    	FETI4IInt* 		indices,
