...
!******************************************************************************
    IMPLICIT NONE
     ...
     TYPE(Variable_t), POINTER ::  WeightIn, WeightsTemp 
...
!-----  -------------------------------------------------------------------------
        ...
        !CHANGE
        !WeightsTemp => VariableGet( Solver % Mesh % Variables, &
        !              'WeightsTemp', UnFoundFatal=.FALSE.)
        !WeightsTemp % Values(:) = WeightValues_tmp(:)

        !IF(ALL(Solver % Matrix % Perm(:) == WeightPerm(:))) THEN
        !  PRINT *, 'UPW TRUE: ',ParEnv % myPE
        !ELSE
        !  PRINT *, 'UPW FALSE: ',ParEnv % myPE
        !END IF 
