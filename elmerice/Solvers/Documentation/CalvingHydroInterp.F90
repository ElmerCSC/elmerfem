...
- **Solver Keywords:**
  ...
  - Load Reader Variables = Logical... (Set this option to true if you’ve got variables read onto secondary hydrology meshes (say, a basal DEM or a runoff raster) that need to be transferred to the main hydrology mesh)
  - Number of Variables To Read = Integer... (If you are loading read-in variables, say how many there are. Note: the solver is currently only set up to deal with a maximum of 10 variables; if you have more than that, you’ll need to modify the source code)
  - Reader Solver 1 = Integer... (If loading variables, say which solver number the reader solver is. Make sure you list as many variables as you've indicated exist to be read in.)
  - Reader V1 = String... (If loading variables, say what the name of the variable is. Make sure you list as many variables as you've indicated exist to be read in.)
