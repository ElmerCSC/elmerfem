...
! Output the sheet discharge (dimension = dimSheet)
   IF (ASSOCIATED(qSol)) THEN
      ...
      ! Mean nodal value on current partition
      WHERE(Refq > 0.0_dp)
         qSolution = qSolution / Refq
      END WHERE

      ! Mean nodal values for nodes that exist on multiple partitions
      CALL ParallelSumVector(Solver % Matrix, qSolution, 3)
...
!> Checking that sheet discharge is flowing from grounde to ocean nodes is 
