...
!Subroutine for predicting calving in 3D
 ...
 SUBROUTINE Find_Calving3D_LSetDistance ( Model, Solver, dt, TransientSimulation )

   USE CalvingGeometry
   USE MainUtils
   USE InterpVarToVar
   ! use modules/LevelSet/LevelSetDistance.F90, has 2 useful subroutines
   !  ExtractZeroLevel() and ComputeDistanceWithDirection()
...
!---------------Get Variables and Parameters for Solution-----------
 ...
 !  CalvingValues = DistValues - 200.0 ! TEMPORARY to test 3D remeshing, this is where, return means stop? !ASK don't need perm because CalvingPerm=DistPerm?
   ! TO DO use LevelSetDistance on PlanMesh and then interpolate for all nodes on 3Dmesh.
  ! RETURN
...
!--------------Generate 2D plane mesh-------------------
   ...
   !---------------------------------------------
    ...
    ELSE
       TotalNodes = NoNodes
    END IF
 ! commented out because depends on extruded levels
...
! TO DO EEF start calculating signed distance here
    ...
    ALLOCATE(WorkReal(NoNodes), WorkPerm(NoNodes))
    WorkReal = 0.0_dp
    WorkPerm = FrontPerm ! this frontperm should be marginsperm!                                    
    CALL VariableAdd(Mesh % Variables, Mesh, Solver, &
         "SignedDistance", 1, WorkReal, WorkPerm)
    NULLIFY(WorkReal, WorkPerm)
    ...
    ! set values for narrowband, dsmax etc!
    DO i=1,Solver % Mesh % NumberOfNodes
         SignDistValues(SignDistPerm(i)) =  ComputeDistanceWithDirection( Solver % Mesh % Nodes % x(i), &
             Solver % Mesh % Nodes % y(i), 0.0d0, DistValues(DistPerm(i)) ) !Dist is old distance to 'level set' (prevfront)
    END DO
    ...
    IF(Boss) WorkReal = IsoMesh % Nodes % z ! TO DO, make workreal just 1 or very large number and then get distance from nonzero HeightVar as levelset.
    DO i=1,SIZE(WorkPerm); WorkPerm(i) = i;
    END DO
! TO DO strategy choice; calculate distance level set on plan mesh or on actual mesh? probably best to calculate on 2D PlanMesh and then interpolate (not first interpolate new surface and then calculate distance based on interpolated values
    CALL VariableAdd(IsoMesh % Variables, IsoMesh, Solver, &
         "CalvingHeight", 1, WorkReal, WorkPerm) ! workreal = z isomesh, which is rotated hence distance between new and old calving front.
...
! commented becasue depends on FNColumns
    ...
    ! TO DO at this point CalvingValues contains a 3 dimensional vector that tells where front node should go, for nodes on planmesh (?)
    ! what happens in IF loop hereafter is mainly catching weird/problematic cases,for a 'nice' calving event, CalvingVar is not adjusted (?)
...
! inspired by LevelSetDistance.F90
  !------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     INTEGER :: i,j,k,n
!------------------------------------------------------------------------------
! TO DO uses previously defined dsmax, narrowband,ZeroLevels,ZeroNodes,AEPS
     ! can use MaxMeshDist which is maximum distance for which calving is foreseen (user defined)
     ! dsmax is maximum levelset change, but how do you know if you don't have the level set yet?
     ! ZeroLevels is nr of triangle elements which contain the zero levelset
     ! ZeroNodes containes the three nodes for each of those trianges
     IF(prevdist - dsmax > narrowband) THEN
       mindist = prevdist - MaxMeshDist 
       RETURN
     ...
     mindist = HUGE(mindist) ! largest number of a kind, largest of mindist?
...
END SUBROUTINE Find_Calving3D_LSetDistance
