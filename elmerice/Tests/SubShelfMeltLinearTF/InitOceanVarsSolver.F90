...
SUBROUTINE InitOceanVarsSolver( Model, Solver, dt, TransientSimulation )
  ...
  T_val  = GetConstReal(SolverParams, 'temp_oce Value', Found)
  IF (.NOT. Found) CALL FATAL('InitOceanVarsSolver', 'temp_oce Value not found')
  S_val  = GetConstReal(SolverParams, 'sal_oce Value', Found)
  IF (.NOT. Found) CALL FATAL('InitOceanVarsSolver', 'sal_oce Value not found')
  ...
  Toce_var  => VariableGet(Solver % Mesh % Variables, 'temp_oce')
  Soce_var  => VariableGet(Solver % Mesh % Variables, 'sal_oce')
  ...
  IF (.NOT. ASSOCIATED(Toce_var)) CALL FATAL('InitOceanVarsSolver', 'temp_oce not found')
  IF (.NOT. ASSOCIATED(Soce_var)) CALL FATAL('InitOceanVarsSolver', 'sal_oce not found')
