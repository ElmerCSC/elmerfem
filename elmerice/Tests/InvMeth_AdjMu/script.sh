...
`tail -5 Cost_Adjoint_Mu.dat | awk '{print$1}' > foo1.dat`
#Recuperation de la norme de la vitesse: (nrm iter 2 de chaque time step)
`grep "ComputeChange: NS (ITER=2) (NRM,RELC): (" OutputSIF_InvMeth_AdjMu.log | grep "navier-stokes" | awk '{print$6}'  > NRMvel.dat`
