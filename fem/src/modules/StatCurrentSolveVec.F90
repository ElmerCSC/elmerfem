...
!------------------------------------------------------------------------------
  ...
  ! CALL INFO(Caller, 'Adaptive Mesh Refinement is not available in this version of ElmerSolver', Level=7)
...
FUNCTION ElectricBoundaryResidual(Model, Edge, Mesh, Quant, Perm, Gnorm) RESULT(Indicator)
  ...
  !       ------------------------
     ...
     IF (.NOT. stat) &
           CALL FATAL('ElectricBoundaryResidual', 'Electric conductivity not found')
  ...
  !
  !          Electric conductivity at the integration point:
  !          --------------------------------------
...
FUNCTION ElectricEdgeResidual(Model, Edge, Mesh, Quant, Perm) RESULT(Indicator)
  ...
  !          --------------------
        ...
        CALL ListGetRealArray(Material, &
                              'Electric Conductivity', Hwrk, en, Edge % NodeIndexes, stat)
        IF (.NOT. stat) &
              CALL Fatal('CurrentEdgeResidual:', 'Electric Conductivity not found')
  ...
  FUNCTION ElectricInsideResidual(Model, Element, Mesh, Quant, Perm, Fnorm) RESULT(Indicator)
    ...
    !    ---------------------------------
      ...
      IF (.NOT. stat) THEN
        CALL Fatal('CurrentInsideResidual:', 'Electric Conductivity not found')
      END IF
