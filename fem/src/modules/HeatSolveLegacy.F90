...
!------------------------------------------------------------------------------
!> The original scalar-element heat equation solver, kept for reference and as
!> an explicit fallback. "HeatSolve" now names the vectorized/threaded solver
!> (formerly HeatSolveVec); this file is reached either directly via
!> "Procedure = HeatSolveLegacy HeatSolverLegacy(_Init)", or through
!> "Legacy Assembly = Logical True" on a solver naming "HeatSolve" or
!> "HeatSolveVec", both of which front for this one.
!------------------------------------------------------------------------------
...
!> \ingroup Solvers
!------------------------------------------------------------------------------
   RECURSIVE SUBROUTINE HeatSolverLegacy( Model,Solver,Timestep,TransientSimulation )
!------------------------------------------------------------------------------
...
! Which compressibility model is used
	...
	TransientAssembly, Converged, AnyMultiply, NeedFlowSol
     ...
     INTERFACE
        SUBROUTINE HeatSolverLegacy_Boundary_Residual( Model,Edge,Mesh,Quant,Perm,Gnorm,Indicator)
          USE Types
          TYPE(Element_t) :: Edge
          TYPE(Model_t) :: Model
          TYPE(Mesh_t) :: Mesh
          ...
          INTEGER :: Perm(:)
        END SUBROUTINE HeatSolverLegacy_Boundary_Residual
        ...
        SUBROUTINE HeatSolverLegacy_Edge_Residual( Model,Edge,Mesh,Quant,Perm,Indicator)
          ...
          INTEGER :: Perm(:)
        END SUBROUTINE HeatSolverLegacy_Edge_Residual

        SUBROUTINE HeatSolverLegacy_Inside_Residual( Model,Element,Mesh,Quant,Perm, Fnorm,Indicator)
          USE Types
          ...
          INTEGER :: Perm(:)
        END SUBROUTINE HeatSolverLegacy_Inside_Residual
     END INTERFACE
   ...
   IF ( ListGetLogical( Solver % Values, 'Adaptive Mesh Refinement', Found ) ) THEN
     IF(.NOT. ListGetLogical( Solver % Values,'Library Adaptivity',Found )) THEN
       CALL RefineMesh( Model,Solver,Temperature,TempPerm, &
           HeatSolverLegacy_Inside_Residual, HeatSolverLegacy_Edge_Residual, &
...
!------------------------------------------------------------------------------
  END SUBROUTINE HeatSolverLegacy
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
   SUBROUTINE HeatSolverLegacy_Inside_Residual( Model, Element, Mesh, &
