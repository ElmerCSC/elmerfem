...
! *
! *  "Permittivity Of Vacuum" convention: this file defaults it to the real
! *  physical value (8.854187817e-12, SI units) when a sif does not set it, so
! *  the equation is solved in absolute units, "eps_0*eps_r*div(grad(V)) =
! *  source", with eps_0 multiplying the left-hand side like any other
! *  material coefficient. A sif that sets "Permittivity Of Vacuum" explicitly
! *  always wins over this default. StatElecSolveVec.F90's thin front injects
! *  the same physical value, so its behavior is unaffected by which name a
! *  sif uses. An Info banner at solver start reports which value is actually
! *  in effect.
! *
...
!------------------------------------------------------------------------------
  ...
  ! "Permittivity Of Vacuum" banner: this file defaults it to the real
  ! physical value (SI units) when a sif does not set it. Report which is
  ! actually in effect so the physical setup is never silently ambiguous.
  BLOCK
    REAL(KIND=dp) :: EpsBanner
    LOGICAL :: FoundBanner

    FoundBanner = .FALSE.
    EpsBanner = PhysicalEps0
    ...
    ELSE
      WRITE( Message,'(A,ES13.6,A)' ) '"Permittivity Of Vacuum" not set: defaulting to ', &
          PhysicalEps0,' (SI units)'
      CALL Info(Caller,Message,Level=4)
    END IF
  END BLOCK
