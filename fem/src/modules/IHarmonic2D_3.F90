...
!------------------------------------------------------------------------------
  ...
  IF (Tstep /= GetTimestep()) THEN
    ...
    IF(ASSOCIATED(LagrangeVar)) THEN
      IF(SIZE(LagrangeVar % Values)>=Circuit_tot_n) ip=LagrangeVar % Values(1:2*Circuit_tot_n)
    END IF
...
!------------------------------------------------------------------------------
    INTEGER :: i,j,k,p,q,n,temp,cnt(Parenv % PEs), r_cnt(ParEnv % PEs)
...
!   return ! xxxx
    ...
    ! attatch components 'vvar' to task that holds the most elements of vvars 'body':
    ! -------------------------------------------------------------------------------
    DO p=1,n_Circuits
    ...
    DO p=1,n_Circuits
      DO CompId=1,Circuits(p) % n_comp
        ...
        CASE('stranded')
          CALL CountCmplxMatElement(Rows, Cnts, RowId, 1)
          IF (Cvar % Owner == ParEnv % myPE) CALL CountCmplxMatElement(Rows, Cnts, RowId, 1)
        CASE('massive')
          CALL CountCmplxMatElement(Rows, Cnts, RowId, 1)
          IF (CVar % Owner == ParEnv % myPE) CALL CountCmplxMatElement(Rows, Cnts, RowId, 1)
        CASE('foil winding')
...
!------------------------------------------------------------------------------
    ...
    DO p = 1,n_Circuits
      DO i=1,Circuits(p) % n
        ...
        vphi=0._dp
        IF (ASSOCIATED(BF).AND.Circuits(p) % SourceRe(i) /= ' ' ) &
        ...
        IF (ASSOCIATED(BF).AND.Circuits(p) % SourceIm(i) /= ' ' ) &
