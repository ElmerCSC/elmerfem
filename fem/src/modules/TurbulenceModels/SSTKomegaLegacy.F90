...
!------------------------------------------------------------------------------
!> The original scalar-element SST K-omega solver, kept for reference and as
!> an explicit fallback. "SSTKomega"/"SSTKOmega" now names the vectorized/
!> threaded solver in SSTKomega.F90; this file is reached either directly via
!> "Procedure = SSTKomegaLegacy SSTKOmegaLegacy(_Init)", or through "Legacy
!> Assembly = Logical True" on a solver naming "SSTKomega" (see
!> SSTKOmegaFront there).
!------------------------------------------------------------------------------
...
!> \ingroup Solvers
!------------------------------------------------------------------------------
   SUBROUTINE SSTKOmegaLegacy( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
     USE DefUtils
...
!------------------------------------------------------------------------------
     ...
     ! LCondensate, which this mirrors (also mirrored in KESolver.F90 and
     ! V2FSolver.F90). Indexed by Element % ElementIndex with stride
     ! bxStride = DOFs*Mesh % MaxBDOFs (not DOFs*nb of any one element, so
     ! blocks stay aligned on a mesh with mixed bubble counts).
...
!------------------------------------------------------------------------------
  END SUBROUTINE SSTKOmegaLegacy
!------------------------------------------------------------------------------

!------------------------------------------------------------------------------
!> Initialization for the primary solver: SSTKOmegaLegacy
!> \ingroup Solvers
!------------------------------------------------------------------------------
   SUBROUTINE SSTKOmegaLegacy_Init( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
     USE DefUtils
...
!------------------------------------------------------------------------------
     ...
     IF ( PBubble ) THEN
       ...
       ! V2F_LDM_Init and IncompressibleNSVec already do for their own
       ! bubbles; CondensatePTransient below makes that choice work for
       ! transient runs too. ListAddNew, so an explicit sif setting still
       ! wins.
       ...
       ! CondensatePTransient in SSTKOmega, mirroring IncompressibleNSVec's
     ...
     END IF
!------------------------------------------------------------------------------
   END SUBROUTINE SSTKOmegaLegacy_Init
!------------------------------------------------------------------------------
