...
!------------------------------------------------------------------------------
!> The original scalar-element static current conduction solver, kept for
!> reference and as an explicit fallback. "StatCurrentSolve" now names the
!> vectorized/threaded solver (formerly StatCurrentSolveVec); this file is
!> reached either directly via "Procedure = StatCurrentSolveLegacy
!> StatCurrentSolverLegacy(_Init)", or through "Legacy Assembly = Logical True"
!> on a solver naming "StatCurrentSolve" or "StatCurrentSolveVec", both of
!> which front for this one.
!------------------------------------------------------------------------------
     ...
     IF (ListGetLogical(Params, 'Adaptive Mesh Refinement', GotIt)) THEN
       IF(.NOT. ListGetLogical(Params,'Library Adaptivity',GotIt)) THEN
         CALL RefineMesh(Model, Solver, Solver % Variable % Values, Solver % Variable % Perm, &
             StatCurrentSolverLegacy_Inside_Residual, StatCurrentSolverLegacy_Edge_Residual, &
             StatCurrentSolverLegacy_Boundary_Residual)
       END IF
     END IF
...
!------------------------------------------------------------------------------
 END SUBROUTINE StatCurrentSolverLegacy
!------------------------------------------------------------------------------
