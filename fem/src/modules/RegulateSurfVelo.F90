...
!---------------------------------------------------------------
  ...
  LOGICAL :: Visited=.FALSE., Found, Top, Bot
  TYPE(Nodes_t) :: Nodes
  TYPE(Element_t), POINTER :: Element
  TYPE(ValueList_t), POINTER :: BC, Params
  INTEGER :: i1,i2,t
  REAL(KIND=dp) :: e1(3), e2(3), Amat(3,3), velem(3,4), v(3), c(3)
  ...
  IF(.NOT. Visited) THEN
    ...
    DO t=Mesh % NumberOfBulkElements+1,&
      ...
      Top = ListGetLogical(BC,'Top Surface',Found)
      Bot = ListGetLogical(BC,'Bot Surface',Found)
      IF( Top .OR. Bot) THEN
