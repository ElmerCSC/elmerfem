...
SUBROUTINE StatCurrentSolver_Init( Model,Solver,dt,TransientSimulation)
  ...
  !         Solver % Variable % Norm = ControlScaling * Solver % Variable % Norm
       ...
       IF ( ListGetLogical( Params, 'Adaptive Mesh Refinement', GotIt ) ) &
          ! CALL Info( 'StatCurrentSolver', 'Adaptive Mesh Refinement is not supported', Level=4 )
  ...
  FUNCTION ElectricBoundaryResidual( Model, Edge, Mesh, Quant, Perm, Gnorm ) &
    ...
    ALLOCATE( EdgeBasis(En), Basis(Pn), dBasisdx(Pn,3), Flux(En), &
      x(En), y(En), z(En), NodalConductivity(En), Potential(Pn) )
    ...
    !       --------------
      Flux(1:En) = ListGetReal( Model % BCs(j) % Values, &
          'Current Flux', En, Edge % NodeIndexes, Found )     
    ...
    !       ------------------------
      ...
      IF ( .NOT. stat )  &
          CALL Fatal('ElectricBoundaryResidual', 'Electric Conductivity not defined.')
  ...
  FUNCTION ElectricEdgeResidual( Model, Edge, Mesh, Quant, Perm ) RESULT( Indicator )
   ...
   !------------------------------------------------------------------------------
    ...
    ALLOCATE( x(En), y(En), z(En), NodalConductivity(En), EdgeBasis(En), &
    ...
    DO t=1,IntegStuff % n
      ...
      !          -----------------------------------------
          ...
          CALL ListGetRealArray( Model % Materials(k) % Values, &
               'Electric Conductivity', Hwrk, En, Edge % NodeIndexes, stat )
          IF ( .NOT. stat )  &
               CALL Fatal('ElectricEdgeResidual', 'Electric Conductivity not defined.')
          ...
          !          ------------------
          DO j=1,dim
             Grad(j,i) = Conductivity * SUM( dBasisdx(1:Pn,j) * Potential(1:Pn) )
          END DO
       END DO
    ...
    DEALLOCATE( x, y, z, NodalConductivity, EdgeBasis, Basis, &
               dBasisdx, Potential )
  !------------------------------------------------------------------------------
  END FUNCTION ElectricEdgeResidual
  !------------------------------------------------------------------------------
...
!    -----------
 ...
 Fnorm     = 0.0_dp
!
!    Check if this equation is computed in this element:
!    ---------------------------------------------------
 IF ( ANY( Perm( Element % NodeIndexes ) <= 0 ) ) RETURN
...
!    ---------------------
 ...
 ALLOCATE( Nodes % x(n), Nodes % y(n), Nodes % z(n), &
   NodalConductivity(n), Basis(n), dBasisdx(n,3),    &
...
!
!    Material parameters: electrical conductivity
!    --------------------------------------------
 k = ListGetInteger( Model % Bodies(Element % BodyId) % Values, 'Material', &
                 minv=1, maxv=Model % NumberOfMaterials )
 ...
 CALL ListGetRealArray( Model % Materials(k) % Values, &
      'Electric Conductivity', Hwrk, n, Element % NodeIndexes, stat )
 IF ( .NOT. stat )  &
      CALL Fatal('ElectricInsideResidual', 'Electric Conductivity not defined.')
...
!
!       Residual of the current conduction equation:
!
!        R = -div(σ grad(u)) - S
!       ---------------------------------------------------
!
...
!    Fnorm = Element % hK**2 * Fnorm
 ...
 DEALLOCATE( Nodes % x, Nodes % y, Nodes % z, NodalConductivity, &
