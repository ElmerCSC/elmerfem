...
#include "../config.h"
   ...
   USE MeshUtils, ONLY : CommunicateParallelSystemTag, CylinderFit, &
       DisplaceMesh, FindMeshEdges, GetLagrangeIndexes, &
       IntegralProjector, MakePermUsingMask
   USE MortarUtils, ONLY : PeriodicProjector, SaveProjector, FindExtremumNodes
   USE ParallelUtils
...
!        END IF
         ...
         DO l=Atmp % Rows(i),Atmp % Rows(i+1)-1
           ...
           IF( Reorder ) THEN               
             IF( col <= dofs*permsize ) THEN
               col2 = (col-1) / dofs + 1
               IF(IsDg) THEN
                 col2 = Perm(DgSome(col2))
               ELSE
                 col2 = Perm(col2)
               END IF
               IF( col2 == 0 ) CYCLE
               col2 = dofs * (col2-1) + mod(col-1,dofs) + 1
             ELSE
               CALL Fatal(Caller,'col index too large: '//I2S(col)//' vs '//I2S(permsize))
             END IF
           ELSE
