    ...
    DO j=1,NoPartitions
      ...
      IF ( ASSOCIATED(Particles % Force) ) THEN
        DO k=1,dim
          DO l=1,n
            m = m + 1
            Buf(m) = Particles % Force(Info(j) % Particles(l),k)
          END DO
        END DO
      END IF
      ...
      IF ( ASSOCIATED(Particles % PrevCoordinate) ) THEN
        DO k=1,dim
          DO l=1,n
            m = m + 1
            Buf(m) = Particles % PrevCoordinate(Info(j) % Particles(l),k)
          END DO
        END DO
      END IF
      ...
      IF ( ASSOCIATED(Particles % PrevVelocity) ) THEN
        DO k=1,dim
          DO l=1,n
            m = m + 1
            Buf(m) = Particles % PrevVelocity(Info(j) % Particles(l),k)
          END DO
        END DO
      END IF
      ...
      DO WHILE( ASSOCIATED(Var) )
        IF( Var % Dofs == 1 ) THEN
          DO l=1,n
            m = m + 1
            Buf(m) = Var % Values(Info(j) % Particles(l))
          END DO
        END IF
        Var => Var % Next 
      END DO
      ...
      IF( ncompInt >  0 ) THEN
        ...
        IF ( ASSOCIATED(Particles % NodeIndex) ) THEN
          DO l=1,n
            m = m + 1
            BufInt(m) = Particles % NodeIndex(Info(j) % Particles(l))
          END DO
        END IF
        ...
        IF ( ASSOCIATED(Particles % Partition) ) THEN
          DO l=1,n
            m = m + 1
            BufInt(m) = Particles % Partition(Info(j) % Particles(l))
          END DO
        END IF
...
!------------------------------------------------------------------------------
      ...
      DO WHILE( ASSOCIATED(Var) )
        k = Var % NameLen
        IF( k > 0 ) THEN
          ! print *,'Increasing size of: '//TRIM(tmp % name(1:k))
          IF( Var % Dofs > 1 ) THEN
            CALL Fatal('ParticleVariableResize','Implement for vectors!')
          END IF
          Values => Var % Values
