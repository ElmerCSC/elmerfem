...
CONTAINS
  ...
  FUNCTION TriangleL(node, u, v) RESULT(fval)
    ...
    REAL(KIND=dp) :: fval
    REAL(KIND=dp), PARAMETER :: c = 1D0/2D0, d = 1D0/SQRT(3D0)
    !$OMP DECLARE SIMD(TriangleL) UNIFORM(node) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(TriangleL) UNIFORM(node) NOTINBRANCH SIMDLEN(8)
  ...
  FUNCTION QuadL(node, u, v) RESULT(fval)
    ...
    !$OMP DECLARE SIMD(QuadL) UNIFORM(node) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(QuadL) UNIFORM(node) NOTINBRANCH SIMDLEN(8)
  ...
  FUNCTION dQuadL(node) RESULT(grad)
    ...
    REAL(KIND=dp) :: grad(2)
    REAL(KIND=dp), PARAMETER :: c = 1/2D0
    !$OMP DECLARE SIMD(dQuadL) UNIFORM(node) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(dQuadL) UNIFORM(node) NOTINBRANCH SIMDLEN(8)
  ...
  FUNCTION TetraL(node, u, v, w) RESULT(fval)
    ...
    REAL(KIND=dp), PARAMETER :: c = 1D0/2D0, d = 1D0/SQRT(3D0), &
            e = 1D0/SQRT(6D0), f = 1D0/SQRT(8D0)
    !$OMP DECLARE SIMD(TetraL) UNIFORM(node) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(TetraL) UNIFORM(node) NOTINBRANCH SIMDLEN(8)
  ...
  FUNCTION WedgeL(node, u, v) RESULT(fval)
    ...
    ! Result
    REAL(KIND=dp) :: fval
    !$OMP DECLARE SIMD(WedgeL) UNIFORM(node) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(WedgeL) UNIFORM(node) NOTINBRANCH SIMDLEN(8)
  ...
  FUNCTION Phi(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(Phi) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(Phi) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
...
!!!                * x ** 2) * x ** 2) * x ** 2) * x ** 2) * x ** 2
  ...
  FUNCTION dPhi(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(dPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(dPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
...
!!!                x ** 2) * x ** 2) * x ** 2) * dble(x) * sqrt(0.78D2) / 0.131072D6
  ...
  FUNCTION VarPhi(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(VarPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(VarPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
...
!!!                x ** 2) * x ** 2) * x ** 2) * sqrt(0.78D2) / 0.131072D6
  ...
  FUNCTION dVarPhi(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(dVarPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(dVarPhi) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
...
!!!                x ** 2) * x ** 2) * x ** 2)
  ...
  FUNCTION LegendreP(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(LegendreP) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(LegendreP) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
...
!!!                x ** 2) * x ** 2
  ...
  FUNCTION dLegendreP(k, x) RESULT(fval)
    ...
    ! Return value
    REAL (KIND=dp) :: fval
    !$OMP DECLARE SIMD(dLegendreP) UNIFORM(k) NOTINBRANCH SIMDLEN(4)
    !$OMP DECLARE SIMD(dLegendreP) UNIFORM(k) NOTINBRANCH SIMDLEN(8)
