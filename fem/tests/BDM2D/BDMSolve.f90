...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp) :: Norm, Normal(3), Normal2(3), &
       u, v, w, NormalSign, Edgeh, SolErr, EK, SolNorm
...
!------------------------------------------------------------------------------
  ...
  !--------------------------------------------------------------------
   if (.true.) then  
      ...
      Print *, 'L2 Error of the flux field = ', sqrt(SolErr)/sqrt(SolNorm)
...
!------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
       ...
       C(1:dim,1:dim) = MATMUL( TRANSPOSE(F(1:dim,1:dim)), F(1:dim,1:dim) )
    ...
    DO t=1,IP % n
       ...
       ELSE 
          ...
          tmp(1,1:dim) = RTBasis(1,1:dim)*Load(2,1) + RTBasis(2,1:dim)*Load(2,2) + &
               RTBasis(3,1:dim)*Load(2,3) + RTBasis(4,1:dim)*Load(2,4) + RTBasis(5,1:dim)*Load(2,5) + &
               RTBasis(6,1:dim)*Load(2,6)
