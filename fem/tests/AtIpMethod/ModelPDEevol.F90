...
!> kernels. 
!------------------------------------------------------------------------------
SUBROUTINE AdvDiffSolver_evol( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
  USE DefUtils
...
!------------------------------------------------------------------------------
    ...
    allocate(diffcoeff(ngp))
    DO t=1,ngp
      DetJ(t) = IP % s(t) * Detj(t)
      diffcoeff(t) = ListGetElementReal( &
          DiffCoeff_h, basis(t,:), Element, GaussPoint = t, IP = IP)
...
!------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
      ...
      ! print *, element % elementindex, t, c, ext, F 
...
!------------------------------------------------------------------------------
END SUBROUTINE AdvDiffSolver_evol
!------------------------------------------------------------------------------

!-------------------------------------------------------------------------------
SUBROUTINE fieldmethodproc(Variable, Element, IP, ipind, X)
!-------------------------------------------------------------------------------
  use DefUtils
...
!-------------------------------------------------------------------------------
  integer :: pointlen, m, sx

  sx = size(x,1)
  x(1) = 1.0
  x(2) = 30.0
!-------------------------------------------------------------------------------
END SUBROUTINE fieldmethodproc
!-------------------------------------------------------------------------------
