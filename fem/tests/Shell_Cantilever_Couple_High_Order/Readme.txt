...
Here the problem is solved by using a high-order discretization which is 
currently possible with special surface geometries only. This option needs
a special command "Cartesian Formulation = Logical True". The command 
"Dead Loads = Logical False" is also essential so that the load is defined to
depend on the current deformation.
